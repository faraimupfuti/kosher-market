...
<div class="container py-4" style="max-width:900px">
    @php
        ...
        $blocked = $conversation->isBlockedFor($guard);
        $blockRoute = $guard === 'customer' ? 'chat.block' : 'vendor.chat.block';
        $unblockRoute = $guard === 'customer' ? 'chat.unblock' : 'vendor.chat.unblock';
        $reportRoute = $guard === 'customer' ? 'chat.report' : 'vendor.chat.report';
    @endphp
    <div class="card shadow-sm border-0 overflow-hidden">
        <div class="card-header bg-dark text-white d-flex justify-content-between align-items-center py-3 gap-2">
            <div>
                <strong>{{ $name }}</strong>
            ...
            <div class="d-flex gap-2">
                ...
                @if($blocked)
                    <form method="POST" action="{{ route($unblockRoute, $conversation) }}">@csrf<button class="btn btn-sm btn-warning">Unblock</button></form>
                @else
                    <form method="POST" action="{{ route($blockRoute, $conversation) }}">@csrf<button class="btn btn-sm btn-outline-warning">Block</button></form>
                @endif
                <button class="btn btn-sm btn-outline-danger" data-bs-toggle="modal" data-bs-target="#reportModal">Report</button>
            </div>
        </div>
        @if($blocked)<div class="alert alert-warning rounded-0 mb-0">You blocked this conversation. Unblock it to send messages.</div>@endif
            ...
            @forelse($conversation->messages as $message)
                ...
                <div class="d-flex mb-3 {{ $mine ? 'justify-content-end' : 'justify-content-start' }}" data-message-id="{{ $message->id }}">
                    <div class="p-3 rounded-3 {{ $mine ? 'bg-primary text-white' : 'bg-white border' }}" style="max-width:78%">
                        ...
                        @if($message->attachment_path)<a class="d-inline-block mt-2 {{ $mine ? 'text-white' : '' }}" href="{{ route($downloadRoute, $message) }}"><i class="fa-solid fa-paperclip me-1"></i>{{ $message->attachment_name ?: 'Attachment' }}</a>@endif
...
<div class="modal fade" id="reportModal" tabindex="-1" aria-hidden="true"><div class="modal-dialog"><div class="modal-content"><form method="POST" action="{{ route($reportRoute, $conversation) }}">@csrf<div class="modal-header"><h5 class="modal-title">Report conversation</h5><button type="button" class="btn-close" data-bs-dismiss="modal"></button></div><div class="modal-body"><label class="form-label">Reason</label><select name="reason" class="form-select" required><option value="spam">Spam</option><option value="harassment">Harassment</option><option value="fraud">Fraud</option><option value="illegal_content">Illegal content</option><option value="other">Other</option></select><label class="form-label mt-3">Details</label><textarea name="details" class="form-control" rows="4" maxlength="2000"></textarea></div><div class="modal-footer"><button type="submit" class="btn btn-danger">Submit report</button></div></form></div></div></div>
@endsection
...
(function () {
    ...
    file.addEventListener('change',()=>{fileName.textContent=file.files.length?file.files[0].name:'';});box.scrollTop=box.scrollHeight;poll();const timer=setInterval(poll,3000);window.addEventListener('beforeunload',()=>clearInterval(timer));
})();
</script>
@endif
@endsection
