...
class Vendor extends Authenticatable
{
    ...
    protected static function booted(): void
    {
        static::saving(function (Vendor $vendor) {
            ...
            if ($vendor->exists && $vendor->isDirty('pseudonym')) {
                $vendor->pseudonym_changed_at = now();
            }
    ...
    {
        return $this->hasManyThrough(ProductReview::class, Product::class, 'vendor_id', 'product_id', 'id', 'id')->where('product_reviews.is_approved', true);
    }
    ...
    {
        if ($this->verification_status === 'verified' && $this->trust_score >= 85) {
            return 'Verified Vendor';
        }
        if ($this->trust_score >= 75) {
