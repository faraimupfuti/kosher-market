...
class VendorTrustService
{
    public function score(Vendor $vendor): int
    {
        ...
        $resolvedAgainst = (int) DB::table('disputes')->where('vendor_id', $vendor->id)->where('status', 'resolved')->where('resolution', 'buyer_refund')->count();
        $refunds = (int) DB::table('escrows')->where('vendor_id', $vendor->id)->whereIn('status', ['refund_pending', 'refunded'])->count();
