...
class BitcoinEscrowService
{
    ...
    private function btcToSatoshis(string|int|float $amount): int
    {
        ...
        if ($satoshis > self::MAX_BTC_SATOSHIS) {
    ...
    public function createForOrder(Order $order): EscrowTransaction
    {
        ...
        if (! $vendorId || ! $buyerId) {
            throw new RuntimeException('The order must have a customer and vendor.');
        }
        if (strtoupper((string) ($order->currency ?? '')) !== 'BTC') {
            throw new RuntimeException('Kosher Market accepts Bitcoin only.');
        }
        $satoshis = $this->btcToSatoshis($order->total_amount);
        if ($satoshis <= 0) {
            throw new RuntimeException('Invalid BTC escrow amount.');
        }
    ...
    public function createBitcoinInvoice(EscrowTransaction $escrow): array
    {
        if ($escrow->currency !== 'BTC') {
            throw new RuntimeException('Bitcoin is the only supported currency.');
        }
    ...
    public function markFunded(EscrowTransaction $escrow, string $txid, int $confirmations, string|int|float $btcAmount): EscrowTransaction
    {
        ...
        if ($paymentSatoshis !== $escrowSatoshis) {
            throw new RuntimeException('Bitcoin payment amount does not exactly match the escrow invoice amount.');
        }
        ...
        return DB::transaction(function () use ($escrow, $txid, $confirmations, $btcAmount) {
            ...
            if (in_array($escrow->status, ['released', 'paid', 'refund_pending', 'refunded', 'cancelled'], true)) {
                return $escrow;
            }
            if ($escrow->bitcoin_txid && ! hash_equals($escrow->bitcoin_txid, $txid)) {
                throw new RuntimeException('Escrow already has a different transaction.');
            }
            ...
            if (! $escrow->ledgerEntries()->where('reference', $txid)->exists()) {
                EscrowLedgerEntry::create(['escrow_transaction_id' => $escrow->id, 'type' => 'escrow_funded', 'amount' => $amount, 'currency' => 'BTC', 'reference' => $txid, 'metadata' => ['confirmations' => $confirmations]]);
            }
    ...
    public function release(EscrowTransaction $escrow, ?string $note = null): EscrowTransaction
    {
        return DB::transaction(function () use ($escrow, $note) {
            ...
            if (! $escrow->canTransitionTo('released')) {
                throw new RuntimeException('Only funded or dispute-eligible escrow can be released.');
            }
            if (! $escrow->vendor?->bitcoin_payout_address) {
                throw new RuntimeException('Vendor has not configured a Bitcoin payout address.');
            }
            if (! $escrow->vendor?->bitcoin_payout_address_verified_at) {
                throw new RuntimeException('Vendor Bitcoin payout address must be verified before release.');
            }
    ...
    public function refund(EscrowTransaction $escrow, ?string $note = null): EscrowTransaction
    {
        return DB::transaction(function () use ($escrow, $note) {
            $escrow = EscrowTransaction::whereKey($escrow->id)->lockForUpdate()->firstOrFail();
            if (! $escrow->canTransitionTo('refund_pending')) {
                throw new RuntimeException('Only funded or disputed escrow can be refunded.');
            }
    ...
    public function submitSettlement(BitcoinSettlement $settlement): BitcoinSettlement
    {
        ...
        if (! $settlement->destination_address) {
            throw new RuntimeException('Settlement destination is missing.');
        }
        if (! preg_match('/^(bc1[ac-hj-np-z02-9]{11,87}|[13][a-km-zA-HJ-NP-Z1-9]{25,34})$/', $settlement->destination_address)) {
            throw new RuntimeException('Only valid Bitcoin mainnet addresses are accepted.');
        }
    ...
    public function approveSettlement(BitcoinSettlement $settlement): BitcoinSettlement
    {
        if (! $settlement->btcpay_payout_id) {
            throw new RuntimeException('Settlement has not been submitted to BTCPay.');
        }
        [$baseUrl, $storeId, $apiKey] = $this->btcpayConfig();
        $current = Http::timeout(20)->withToken($apiKey)->acceptJson()->get($baseUrl.'/api/v1/payouts/'.$settlement->btcpay_payout_id);
        if ($current->failed()) {
            throw new RuntimeException('Unable to retrieve the BTCPay payout revision.');
        }
        ...
        if ($response->failed()) {
            throw new RuntimeException('BTCPay payout approval failed.');
        }
    ...
    {
        if (! $settlement->btcpay_payout_id) {
            return $this->submitSettlement($settlement);
        }
        [$baseUrl, $storeId, $apiKey] = $this->btcpayConfig();
        $response = Http::timeout(20)->withToken($apiKey)->acceptJson()->get($baseUrl.'/api/v1/payouts/'.$settlement->btcpay_payout_id);
        if ($response->failed()) {
            throw new RuntimeException('Unable to retrieve BTCPay payout status.');
        }
        $payout = $response->json();
        $state = $this->normalizePayoutState($payout['state'] ?? null);
        ...
        if ($state === 'completed') {
            ...
            if ($escrow) {
                if ($settlement->type === 'seller_payout' && $escrow->canTransitionTo('paid')) {
                    $escrow->update(['status' => 'paid']);
                }
                if ($settlement->type !== 'seller_payout' && $escrow->canTransitionTo('refunded')) {
                    $escrow->update(['status' => 'refunded', 'refunded_at' => $escrow->refunded_at ?: now()]);
                }
            }
        }
