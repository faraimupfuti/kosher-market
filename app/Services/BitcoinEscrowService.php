...
class BitcoinEscrowService
{
    ...
    public function markFunded(EscrowTransaction $escrow, string $txid, int $confirmations, float $btcAmount): EscrowTransaction
    {
        ...
        return DB::transaction(function () use ($escrow, $txid, $confirmations, $btcAmount) {
            ...
            $escrow->update(['status' => 'funded', 'bitcoin_txid' => $txid, 'bitcoin_amount' => $btcAmount, 'bitcoin_confirmations' => $confirmations, 'payment_detected_at' => $escrow->payment_detected_at ?: now(), 'payment_confirmed_at' => now(), 'funded_at' => $escrow->funded_at ?: now(), 'release_due_at' => $escrow->release_due_at ?: now()->addDays((int) config('escrow.hold_days', 3))]);
            if (!$escrow->ledgerEntries()->where('reference', $txid)->exists()) EscrowLedgerEntry::create(['escrow_transaction_id' => $escrow->id, 'type' => 'escrow_funded', 'amount' => $btcAmount, 'currency' => 'BTC', 'reference' => $txid, 'metadata' => ['confirmations' => $confirmations]]);
            return $escrow;
        });
    }
    ...
    private function normalizePayoutState(?string $state): string
    {
        return match (strtolower(str_replace(['-', ' '], '_', (string) $state))) {
            ...
            'awaitingpayment', 'awaiting_payment' => 'awaiting_payment',
