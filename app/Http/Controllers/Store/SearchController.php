...
class SearchController extends Controller
{
    ...
    private function productQuery(array $data)
    {
        ...
        if (!empty($data['category_id'])) $builder->where('category_id', $data['category_id']);
        if (!empty($data['vendor_id'])) $builder->where('vendor_id', $data['vendor_id']);
        if (isset($data['min_price'])) $builder->where('price', '>=', $data['min_price']);
        if (isset($data['max_price'])) $builder->where('price', '<=', $data['max_price']);
        if (isset($data['min_rating'])) $builder->having('reviews_avg_rating', '>=', $data['min_rating']);
    ...
    public function suggestions(Request $request)
    {
        ...
        $query = trim($data['q']); $locale = $data['locale'] ?? App::getLocale();
