...
class EscrowController extends Controller
{
    ...
    public function create(Request $request, Order $order)
    {
        ...
        try { return response()->json($this->escrow->createForOrder($order)->load(['order', 'vendor']), 201); }
        catch (Throwable $e) { report($e); return response()->json(['message' => 'Unable to create escrow for this order.'], 422); }
    }
    ...
    public function createPayment(Request $request, EscrowTransaction $escrow)
    {
        ...
        try { $invoice = $this->escrow->createBitcoinInvoice($escrow); return response()->json(['escrow_id'=>$escrow->id,'currency'=>'BTC','invoice_id'=>$invoice['id']??null,'checkout_url'=>$invoice['checkoutLink']??null,'payment_address'=>data_get($invoice,'addresses.BTC')]); }
    ...
    public function dispute(Request $request, EscrowTransaction $escrow)
    {
        ...
        $dispute=DB::transaction(function()use($escrow,$customer,$data){$locked=EscrowTransaction::whereKey($escrow->id)->lockForUpdate()->firstOrFail();if(!in_array($locked->status,['funded','processing'],true))abort(422,'This escrow cannot be disputed.');$dispute=EscrowDispute::create(['escrow_transaction_id'=>$locked->id,'opened_by'=>$customer->id,'reason'=>$data['reason'],'description'=>$data['description'],'status'=>'open']);$locked->transitionTo('disputed');$locked->save();return $dispute;});
    ...
    public function webhook(Request $request)
    {
        ...
        if (!$type) return response()->json(['message'=>'Webhook event type is required.'],422);
        ...
        try {
            ...
            report($e); return response()->json(['message'=>'Unable to record webhook event.'],500);
        }
        ...
        try {
            ...
            if (($invoice['id'] ?? $invoiceId) !== $invoiceId) throw new \RuntimeException('BTCPay returned a different invoice identity.');
            if (($invoice['status'] ?? '') !== 'Settled') { $event->update(['processed_at'=>now()]); return response()->json(['received'=>true]); }
            ...
            if (($metadata['kosherMarketType'] ?? null) === 'vendor_registration_fee') {
                ...
                if (!$fee) throw new \RuntimeException('Vendor registration invoice is not bound to an existing fee record.');
            ...
            if (!$escrow) throw new \RuntimeException('BTCPay invoice is not bound to an existing escrow.');
            if (!$escrow->btcpay_invoice_id || !hash_equals((string)$escrow->btcpay_invoice_id,(string)$invoiceId)) throw new \RuntimeException('BTCPay invoice does not match the escrow invoice binding.');
            if (strtoupper((string)($invoice['currency']??'')) !== 'BTC') throw new \RuntimeException('Escrow invoice currency is not BTC.');
            if ($escrow->currency !== 'BTC') throw new \RuntimeException('Escrow currency is not BTC.');
            ...
            if ($txid === '') throw new \RuntimeException('Settled BTCPay invoice has no transaction identifier.');
