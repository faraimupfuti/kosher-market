...
class EscrowController extends Controller
{
    ...
    public function dispute(Request $request, EscrowTransaction $escrow)
    {
        ...
        $dispute = DB::transaction(function () use ($escrow, $customer, $data) {
            ...
            $locked->update(['status'=>'disputed']); return $dispute;
        });
    ...
    public function webhook(Request $request)
    {
        ...
        if (in_array($type, ['PayoutApproved','PayoutUpdated'], true)) {
            $payoutId = $payload['payoutId'] ?? null;
            if ($payoutId) {
                $settlement = BitcoinSettlement::where('btcpay_payout_id', $payoutId)->first();
                if ($settlement) {
                    try { $this->escrow->syncSettlement($settlement); } catch (Throwable $e) { report($e); return response()->json(['message'=>'Unable to synchronize payout.'],502); }
                }
            }
