...
class BitcoinSettlementController extends Controller
{
    public function index()
    {
        ...
        $vendors = Vendor::whereNotNull('bitcoin_payout_address')->orderBy('name')->get([
            'id', 'name', 'email', 'bitcoin_payout_address', 'bitcoin_payout_address_verified_at',
        ]);
    ...
    {
        if (! $vendor->bitcoin_payout_address || ! preg_match('/^(bc1[ac-hj-np-z02-9]{11,87}|[13][a-km-zA-HJ-NP-Z1-9]{25,34})$/', $vendor->bitcoin_payout_address)) {
            return back()->withErrors(['vendor' => 'Vendor does not have a valid Bitcoin mainnet payout address.']);
        }
    ...
    public function destination(Request $request, BitcoinSettlement $settlement)
    {
        $data = $request->validate([
            'destination_address' => [
                ...
                'string',
                'max:120',
                'regex:/^(bc1[ac-hj-np-z02-9]{11,87}|[13][a-km-zA-HJ-NP-Z1-9]{25,34})$/',
            ],
        ]);

        if ($settlement->status === 'completed') {
            return back()->withErrors(['settlement' => 'Completed settlements cannot be changed.']);
        }

        $settlement->update([
            'destination_address' => trim($data['destination_address']),
            'status' => 'pending',
            'error_message' => null,
        ]);
    ...
    public function submit(BitcoinSettlement $settlement, BitcoinEscrowService $service)
    {
        try {
            ...
            return back()->with('success', 'Payout request submitted to BTCPay. It now requires approval.');
        } catch (Throwable $e) {
    ...
    {
        try {
            $service->approveSettlement($settlement);

            return back()->with('success', 'BTCPay payout approved.');
        } catch (Throwable $e) {
            return back()->withErrors(['settlement' => $e->getMessage()]);
        }
    }
    ...
    public function sync(BitcoinSettlement $settlement, BitcoinEscrowService $service)
    {
        try {
            ...
            return back()->with('success', 'Settlement status synchronized with BTCPay.');
        } catch (Throwable $e) {
            return back()->withErrors(['settlement' => $e->getMessage()]);
        }
    }
